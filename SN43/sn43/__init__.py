...
if not logger.handlers:
    ...
    _h.setFormatter(logging.Formatter("%(asctime)s [%(levelname)s] [sn43] %(message)s"))
    logger.addHandler(_h)
logger.setLevel(getattr(logging, (os.getenv("LOGLEVEL") or "INFO").upper(), logging.INFO))
def _lvl(x):
    ...
    try:
        for candidate in ["/app/tools.py", "/app/agent.py"]:
            if os.path.exists(candidate):
                _load_module_from_file(f"sn43_autoload_{Path(candidate).stem}", Path(candidate))
                logger.info(f"sn43: loaded file {candidate}")
    except Exception as e:
...
def _bootstrap_container_token() -> None:
    ...
    if fixed and fixed not in _TOKEN_META:
        ...
        logger.info("sn43: accepted container sn43_TOKEN for RPC")
...
class Container:
    def __init__(
        ...
        *,
        ...
        self.image = image or "thebes1618/sn43:latest"
        ...
        if self.base_url:
            ...
            if self.local_script_path and os.path.exists(self.local_script_path):
                ...
                # Copy our package into /app so `import sn43` works without pip install
                ...
                exec_in_container_detach(
                    self.container_id,
                    "cd /app && /opt/venv/bin/python -m uvicorn sn43:app --host 0.0.0.0 --port 5005",
                )
            else:
                # Image-only mode: assume image starts uvicorn sn43:app
                self.container_id = start_container(
                    self.image,
                    self.container_name,
                    env={"sn43_TOKEN": self.token},
            ...
            logger.info(f"sn43: container {self.container_id[:12]} listening at {self.base_url}")

            # Wait for health before proceeding
            deadline = time.time() + 120.0
            while time.time() < deadline:
                try:
                    r = requests.get(f"{self.base_url}/healthz", timeout=2.0)
                    if r.ok:
                        logger.info(f"Container {self.container_id[:12]} is healthy")
                        break
                except Exception as e:
                    last_error = str(e)
                time.sleep(1.0)
                ...
                try:
                    ...
                    logger.warning(f"sn43: /methods before fail -> {getattr(r,'text',None)}")
                    ...
                    logger.error(f"Container {self.container_id[:12]} logs:\n{container_logs}")
                except Exception as e:
                    logger.warning(f"sn43: /methods request failed: {e}")
                raise RuntimeError(
                    f"Agent HTTP server did not become ready in time. "
                    f"Last error: {last_error}. Check container logs for details."
                )
